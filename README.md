...
* Al comparar los resultados de `HMM` con respecto a K-Means (Taller 3), se puede apreciar que `HMM`, tanto al usar selección de `SCORE` (Log-Likelihood) como `BIC`, encuentra una mayor cantidad de estados ocultos (comparables con los clústeres del Taller 3). Estos clústeres intuitivamente parecen agrupar de forma correcta la información. Por esta razón, se podría afirmar que `HMM` permite encontrar una mayor cantidad de patrones que K-Means. No se ha logrado generar un escenario de `HMM` bajo el cual uno de sus estados ocultos corresponda a valores atípicos, por lo cual esta parece ser una limitación para las dos técnicas. Revisión de literatura y múltiples escenarios adicionales se requieren para concluir con mayor certeza las ventajas de `HMM` frente a K-Means. En particular dado que las dos técnicas tienen numerosas configuraciones que no se han explorado en este Taller, además, K-Means, podría ser reemplazado otras técnicas no supervisadas como Clústeres Jerarquicos. 
